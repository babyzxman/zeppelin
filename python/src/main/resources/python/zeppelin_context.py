...
class PyZeppelinContext(object):
    ...
    def importNotebook(self, noteId):
        paragraphs = self.z.getParagraphTexts(noteId)
        local_namespace = {}
        for paragraph in paragraphs:
            cleanText = paragraph.replace("%python", "")
            cleanText = cleanText.replace("%pyspark", "")
            exec(cleanText, local_namespace)  # Execute in isolated namespace
        return local_namespace

    def clearNoteTextbox(self):
        return self.z.clearNoteTextbox()
