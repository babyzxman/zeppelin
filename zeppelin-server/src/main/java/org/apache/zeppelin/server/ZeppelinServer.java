...
public class ZeppelinServer {
  ...
  public static void main(String[] args) throws IOException {
    ...
    ServiceLocatorUtilities.bind(
            ...
            new AbstractBinder() {
              ...
              protected void configure() {
                ...
                bindAsContract(SparkObject.class).in(Singleton.class);
                bindAsContract(UserContext.class).in(Singleton.class);
    ...
    boolean isRegisterZeppelinSuccess = false;
    try {
      RestTemplateFactoryUtil.setHttpConnectTimeout(zeppelinConfig.getInt(ConfVars.ZEPPELIN_CONNECTION_TIMEOUT));

      //Initiate zeppelin application info for BDE
      String appName = zeppelinConfig.getString(ConfVars.ZEPPELIN_APP_NAME);
      Integer appPort = Integer.valueOf((zeppelinConfig.getBoolean(ConfVars.ZEPPELIN_SSL) ? zeppelinConfig.getString(ConfVars.ZEPPELIN_SSL_PORT) : zeppelinConfig.getString(ConfVars.ZEPPELIN_PORT)));

      if(zeppelinConfig.getString(ConfVars.ZEPPELIN_APP_HARDWARE_ID) != null && !zeppelinConfig.getString(ConfVars.ZEPPELIN_APP_HARDWARE_ID).trim().isEmpty())
        ApplicationConstant.setModuleReferenceKey(appName,appPort,zeppelinConfig.getString(ConfVars.ZEPPELIN_APP_HARDWARE_ID));
      else
        ApplicationConstant.setModuleReferenceKey(appName,appPort);

      zeppelinConfig.setProperty("zeppelin.app.module.ref.key",ApplicationConstant.getModuleReferenceKey());
      ApplicationConstant.setListNotePageUrl(zeppelinConfig.getString(ConfVars.HERA_PUBLIC_ADDR) +
              ":" + zeppelinConfig.getString(ConfVars.HERA_PUBLIC_PORT)+zeppelinConfig.getString(ConfVars.BDE_LIST_NOTE_URL));

      ApplicationInformation appInfo = new ApplicationInformation();
      appInfo.setGroup(zeppelinConfig.getString(ConfVars.ZEPPELIN_APP_GROUP));
      appInfo.setId(zeppelinConfig.getString(ConfVars.ZEPPELIN_APP_MODULE_ID));
      appInfo.setSecret(zeppelinConfig.getString(ConfVars.ZEPPELIN_APP_PASSWORD));
      appInfo.setName(zeppelinConfig.getString(ConfVars.ZEPPELIN_APP_NAME));
      appInfo.setDescription(zeppelinConfig.getString(ConfVars.ZEPPELIN_APP_DESCRIPTION));
      appInfo.setVersion(Util.getVersion());
      appInfo.setCopyRight(zeppelinConfig.getString(ConfVars.ZEPPELIN_APP_COPYRIGHT));

      String zeppelinUrl = (zeppelinConfig.getString(ConfVars.ZEPPELIN_APP_PREFIX_URL).contains("://")) ? zeppelinConfig.getString(ConfVars.ZEPPELIN_APP_PREFIX_URL) : zeppelinConfig.getString(ConfVars.ZEPPELIN_APP_PREFIX_URL) + "://";
      if(!zeppelinConfig.getString(ConfVars.ZEPPELIN_APP_REGISTER_HOST_IP).isEmpty())
        zeppelinUrl += zeppelinConfig.getString(ConfVars.ZEPPELIN_APP_REGISTER_HOST_IP) +":"+ appPort;
      else
        zeppelinUrl += InetAddress.getLocalHost().getHostAddress()+":"+ appPort;

      appInfo.setUrl(zeppelinUrl);
      appInfo.setTenantId(zeppelinConfig.getLong(ConfVars.ZEPPELIN_APP_TENANT_ID));
      ApplicationConstant.setINFO(appInfo);

      HiveSiteConfig hiveSiteConfig = new HiveSiteConfig();
      hiveSiteConfig.setUrl(zeppelinConfig.getString(ConfVars.ZEPPELIN_APP_HIVE_SITE_URL));
      ApplicationConstant.setHiveSiteConfig(hiveSiteConfig);

      LOG.info("Register and validate Zeppelin license to BDE...");
      //Register and validate Zeppelin license to BDE
      BDEHeraServices bdeHeraServices = new BDEHeraServices();
      isRegisterZeppelinSuccess = bdeHeraServices.registerZeppelinModule();

    }catch (Exception e){
      LOG.error("Error while registering and validate license of zeppelin module to BDE");
      LOG.error(e.getMessage(),e);
      System.exit(-1);
    }

    if (isRegisterZeppelinSuccess) {
      LOG.info("Done, zeppelin server started");
      try {
        //call stop all public notebook
        LOG.info("Start stop all parallel notebooks");
        BDEHeraServices bdeHeraServices = new BDEHeraServices();
        bdeHeraServices.stopParallelNotebook();
      } catch (Exception e) {
        LOG.error("Error When trying to stop all parallel notebook case: " + e.getMessage(), e);
      }
      try {
        LOG.info("Start run all initial notebooks");
        String initialNoteNamesStr = zeppelinConfig.getString(ConfVars.ZEPPELIN_INITIAL_NOTES);
        List<String> initialNoteNameList = new ArrayList<>();
        if (initialNoteNamesStr!= null && !initialNoteNamesStr.isEmpty()) {
          initialNoteNameList = Arrays.asList(initialNoteNamesStr.split(","));
        }
        List<NoteInfo> notesInfo = notebook.getNotesInfo();
        for (String noteName: initialNoteNameList) {
          String noteId = notesInfo.stream().filter(noteInfo -> noteInfo.getNoteName().equals(noteName)).findFirst().get().getId();
          notebook.processNote(noteId,
                  note -> {
                    AuthenticationInfo subject = new AuthenticationInfo("anonymous");
                    subject.setRoles(new HashSet<>());
                    try {
                      note.runAll(subject, false, true, "UNKNOWN",false, new HashMap<>(), new HashMap<>());
                      return new JsonResponse<>(Response.Status.OK).build();
                    } catch (Exception e) {
                      LOG.error("Error When try to process note: " + noteName + " case: " + e.getMessage(), e);
                      return new JsonResponse<>(Response.Status.OK).build();
                    }
                  });
        }
      } catch (Exception e) {
        LOG.error("Error When trying to run initial notes case: " + e.getMessage(), e);
      }
      try {
      ...
      LOG.info("Error when trying to register zeppelin module to BDE.");
      shutdown(conf, jettyWebServer, sharedServiceLocator);
    }
  }
  ...
  private static void runNoteOnStart(ZeppelinConfiguration conf, Server jettyWebServer, ServiceLocator sharedServiceLocator) throws IOException {
    ...
    if (!StringUtils.isEmpty(noteIdToRun)) {
      ...
      boolean success = notebookService.runAllParagraphs(noteIdToRun, null, null, serviceContext, "SYSYEM", new ServiceCallback<Paragraph>() {
        @Override
  ...
  private static void setupRestApiContextHandler(WebAppContext webapp, ZeppelinConfiguration conf) {
    ...
    webapp.addFilter(RestAuthFilter.class, "/api/notebook/*", EnumSet.of(DispatcherType.REQUEST));
