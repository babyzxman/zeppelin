...
public class NotebookService {
  ...
  public String createNote(String notePath,
    ...
    try {
      String noteId = notebook.createNote(normalizeNotePath(notePath), defaultInterpreterGroup,
          context.getAutheInfo(), null, false);
  ...
  public void clearNoteCatch()
          throws IOException {
    notebook.clearNoteRepo();
  }

  public void clearNoteCatchById(List<String> noteIds) throws IOException{
    notebook.clearNoteRepoByNoteIds(noteIds);
  }
  ...
  public void renameNote(String noteId,
    ...
    notebook.processNote(noteId,
      readNote -> {
        ...
        try {
          if(!notebook.isNotePathAvailable(newNotePathReal)){
            throw new NotePathAlreadyExistsException("Note '" + newNotePath + "' existed");
          }

          if(isValidateBDEToken){
            //Call hera to update new note name
            BDEHeraServices heraServices = new BDEHeraServices();
            heraServices.updateNoteRename(token,noteId,sessionId,newNotePath);
          }
  ...
  public String cloneNoteWithParameter(String noteId,
                                       String revisionId,
                                       String newNotePath,
                                       Map<String, Map<String, String>> parameters,
                                       Map<String, String> newInterpreterGroupName,
                                       String language,
                                       ServiceContext context,
                                       ServiceCallback<Note> callback) throws IOException {
    //TODO(zjffdu) move these to Notebook
    ...
    try {
      String newNoteId = notebook.cloneNoteWithParameter(noteId, revisionId, normalizeNotePath(newNotePath),
              parameters, newInterpreterGroupName, language, context.getAutheInfo());
      return notebook.processNote(newNoteId,
              newNote -> {
                callback.onSuccess(newNote, context);
                return newNote.getId();
  ...
  public String importNote(String notePath,
    ...
    try {
      // pass notePath when it is null
      String noteId = notebook.importNote(noteJson,noteLanguage, notePath == null ?
                      notePath : normalizeNotePath(notePath),
              context.getAutheInfo());
      return notebook.processNote(noteId,
              note -> {
  ...
  public boolean runParagraph(Note note,
    ...
    try {
      notebook.saveNote(note, context.getAutheInfo());
      if (scriptText != null && !scriptText.equals(p.getText())) {
      ...
      note -> {
        ...
        if (paragraphs != null) {
          ...
          try {
            ...
            for (Map<String, Object> raw : paragraphs) {
              ...
              try {
                ...
                String scriptText = heraServices.addTokenToHeraApiText(text, nbToken, id, runBy, noteId);
                Map<String, Object> params = (Map<String, Object>) raw.get("params");
                Map<String, Object> config = (Map<String, Object>) raw.get("config");

                if (!runParagraph(note, paragraphId, title, text, scriptText, params, config, null, runBy, false, true,
                        context, callback)) {
          ...
          try {
            ...
            note.runAll(context.getAutheInfo(), true, false, runBy,false, new HashMap<>(), new HashMap<>());
            return true;
          } catch (Exception e) {
...
//          Set<String> requestCronRoles = (Set<String>) config.get("cronExecutingRoles");
          Set<String> requestCronRoles = new HashSet<>();
          if (!config.get("cronExecutingRoles").toString().equals("[]") && !config.get("cronExecutingRoles").toString().equals("")) {
...
//            if (!context.getUserAndRoles().contains(requestCronUser)) {
            if (!context.getUserAndRoles().contains(requestCronUser) && config.get("cron") != null) {
              LOGGER.error("Wrong cronExecutingUser: {}", requestCronUser);
              callback.onFailure(new IllegalArgumentException(requestCronUser), context);
              return null;
            }

            if (!context.getUserAndRoles().containsAll(requestCronRoles) && config.get("cron") != null) {
    ...
    try {
      ...
      notebook.processNote(noteId,
        note -> {
          ...
          if (ObjectUtil.isNullOrEmpty(message.get("runBy"))) {
            p.setRunBy((String) message.get("runBy"));
          }
