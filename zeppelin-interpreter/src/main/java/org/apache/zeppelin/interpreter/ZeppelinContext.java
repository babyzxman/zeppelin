...
public abstract class ZeppelinContext {
  ...
  @ZeppelinApi
  public List<String> getParagraphTexts(String noteId) throws IOException {
    return getParagraphTexts(noteId, interpreterContext);
  }

  public List<String> getParagraphTexts(String noteId, InterpreterContext context) throws IOException {
    List<ParagraphInfo> paragraphInfos =  context.getIntpEventClient()
            .getParagraphList("", noteId);
    return paragraphInfos.stream()
            .map(ParagraphInfo::getParagraphText)
  ...
  public void clearNoteTextbox() {
    Map<String,Object> param = noteGui.getParams();
    param.clear();
    noteGui.setParams(param);
    setNoteGui(noteGui);
  }
