...
public class JobProgressPoller extends Thread {
  public static final long DEFAULT_INTERVAL_MSEC = 2000;
  ...
  public void run() {
    try {
      while (!Thread.interrupted()) {
        ...
        if (listener != null) {
          try {
            if (zeppelinConfig.getBoolean(ZeppelinConfiguration.ConfVars.ZEPPELIN_INTERPRETER_SKIP_ABORT)) {
              if (job.isAborted()) {
                logger.warn("Change status of interpreter from abort to running");
