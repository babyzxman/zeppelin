...
public class ZeppelinConfiguration {
  ...
  public String getAbsoluteDir(String path) {
    if (path != null && (path.startsWith(File.separator) || isWindowsPath(path) || isPathWithScheme(path))) {
      LOGGER.info("Path EXIT: " + path);
      return path;
    } else {
      LOGGER.info("Path NOT EXIT: " + getString(ConfVars.ZEPPELIN_HOME) + File.separator + path);
...
//  Hera Configuration
    HERA_ADDR("hera.server.addr", "https://127.0.0.1"),
    HERA_PORT("hera.server.port", "11010"),
    HERA_PUBLIC_ADDR("hera.server.public.addr", "https://127.0.0.1"),
    HERA_PUBLIC_PORT("hera.server.public.port", "11010"),
    HERA_SSL("hera.ssl", false),
    HERA_USER("hera.user", ""),

//  Zeppelin module configuration
    ZEPPELIN_APP_MODULE_ID("zeppelin.app.module.id",11),
    ZEPPELIN_APP_NAME("zeppelin.app.name","ZEPPELIN"),
    ZEPPELIN_APP_TENANT_ID("zeppelin.app.tenant.id",1),
    ZEPPELIN_APP_HIVE_SITE_URL("zeppelin.app.hive.site.url",""),
//    ZEPPELIN_APP_DATA_STORE_DIR("zeppelin.app.datastore.dir",""),
    ZEPPELIN_APP_DESCRIPTION("zeppelin.app.description",""),
    ZEPPELIN_APP_GROUP("zeppelin.app.group",""),
    ZEPPELIN_APP_PASSWORD("zeppelin.app.password",""),
    ZEPPELIN_APP_COPYRIGHT("zeppelin.app.copyright",""),
    ZEPPELIN_APP_PREFIX_URL("zeppelin.app.prefix.url",""),
    ZEPPELIN_APP_FULL_URL("zeppelin.app.full.url",""),
    ZEPPELIN_APP_MODULE_REF_KEY("zeppelin.app.module.ref.key",""),
    ZEPPELIN_APP_HARDWARE_ID("zeppelin.app.hardware.id",""),
    ZEPPELIN_APP_REGISTER_HOST_IP("zeppelin.app.register.host.ip",""),
    ZEPPELIN_APP_REGISTER_HOST_NAME("zeppelin.app.register.host.name",""),

//  zeppelin-server services url
    ZEPPELIN_SERVER_SEND_HEALTH_STATUS_URL("zeppelin.server.send.health.url",""),
    ZEPPELIN_SERVER_LICENSE_VALIDATE_HOLD_CPU_URL("zeppelin.server.license.validate.hold.cpu.url",""),
    ZEPPELIN_SERVER_LICENSE_RELEASE_CPU_URL("zeppelin.server.license.release.cpu.url",""),
    ZEPPELIN_SERVER_LICENSE_VALIDATE_CPU_URL("zeppelin.server.license.validate.cpu.url",""),
    ZEPPELIN_SERVER_ADD_SPARK_PORT("zeppelin.server.add.spark.port", ""),
    ZEPPELIN_SERVER_SPARK_MEM_OVER_LIMIT_URL("zeppelin.server.spark.mem.over.limit.url", ""),
    ZEPPELIN_SERVER_UPDATE_SPARK_TIMEOUT("zeppelin.server.update.spark.timeout",""),
    ZEPPELIN_BDE_VALIDATE("zeppelin.bde.validate", ""),

//  BDE services url
    BDE_LIST_NOTE_URL("bde.list.note.url","/#/login"),
    HERA_SERVICE_CRON_LICENSE_VALIDATE_URL("hera.validate.cron.license.url",""),
    HERA_SERVICE_CRON_HOLD_LICENSE_CPU_URL("hera.hold.cron.license.cpu.url",""),
    HERA_SERVICE_CRON_UNHOLD_LICENSE_CPU_URL("hera.unhold.cron.license.cpu.url",""),
    HERA_SERVICE_REGISTER_ZEPPELIN_MODULE_URL("hera.register.module.url",""),
    HERA_SERVICE_STOP_ALL_PARALLEL_URL("hera.stop.all.parallel.url",""),
    HERA_SERVICE_VALIDATE_BDE_TOKEN_URL("hera.validate.bde.token.url",""),
    HERA_SERVICE_RENAME_NOTE_URL("hera.rename.note.url",""),
    HERA_SERVICE_SEND_HEALTH_STATUS_URL("hera.send.health.status.url",""),
    HERA_ZEPPELIN_SERVICE_SEND_HEALTH_STATUS_URL("hera.zeppelin.send.health.status.url",""),
    HERA_SERVICE_CHECK_CPU_LICENSE_OVER_LIMIT("hera.check.license.over.limit", ""),
    HERA_SERVICE_LICENSE_CHECK_MODULE_URL("hera.license.check.module.url",""),
    HERA_SERVICE_AUTH_URL("hera.auth.user.url",""),
    HERA_AUTH_NOTEBOOK_URL("hera.auth.notebook.url",""),
    HERA_SERVICE_IMPORT_CALLING("hera.service.import.calling.url",""),
    HERA_SERVICE_SET_DEFAULT_SPARK_INTERPRETER_URL("hera.service.set.default.spark.interpreter.url", ""),

//  zeppelin configuration
    ZEPPELIN_CONNECTION_TIMEOUT("zeppelin.connection.timeout",""),
    SPARK_INTERPRETER_FILE("spark.interpreter.file","./conf/interpreter.json"),
    SPARK_LOCAL_CONFIG_FILE("spark.local.config.file","./conf/spark-local.conf"),
    ZEPPELIN_PUBlIC_HOST_NAME("zeppelin.public.host.name","localhost"),
    ZEPPELIN_PUBlIC_HOST_IP("zeppelin.public.host.ip","127.0.0.1"),
    ZEPPELIN_PUBlIC_PORT("zeppelin.public.port","8998"),
    ZEPPELIN_PUBlIC_DOMAIN_SUB_PATH("zeppelin.public.domain.sub.path",""),
    ZEPPELIN_ENABLE_SPARK_PERMISSION("zeppelin.enable.spark.permission",true),
    ZEPPELIN_ENABLE_SAME_METASTORE("zeppelin.enable.same.metastore",false),
    ZEPPELIN_ENABLE_VIEW_TABLE("zeppelin.enable.view.table",false),


    IPYNB_LANGUAGE_PARENT_KEY("ipynb.language.parent.key","spark"),

    ZEPPELIN_HOME("zeppelin.home", "./"),
    ZEPPELIN_ADDR("zeppelin.server.addr", "127.0.0.1"),
    ZEPPELIN_PORT("zeppelin.server.port", 8080),

    ZEPPELIN_LOCAL_ADDR("zeppelin.server.local.addr", "var"),
    ZEPPELIN_LOCAL_PORT("zeppelin.server.local.port", "var"),
    ...
    ZEPPELIN_INTERPRETER_ENABLE_SCOPED_MODE("zeppelin.interpreter.enable.scoped.mode", false),
    ZEPPELIN_INTERPRETER_SPARK_SCOPED_LIST("zeppelin.interpreter.spark.scoped.list", ""),
    ZEPPELIN_INTERPRETER_SPARK_SCOPED_CLEAR_LIFECYCLE("zeppelin.interpreter.spark.scoped.clear.lifecycle", 3600000L),
    ZEPPELIN_INTERPRETER_GROUP_DEFAULT("zeppelin.interpreter.group.default", "spark"),
    ZEPPELIN_PYTHON_ADD_INTERPRETER("zeppelin.python.add.interpreter",true),
    ...
    ZEPPELIN_INITIAL_NOTES("zeppelin.initial.notes", ""),
    ...
    ZEPPELIN_WEBSOCKET_TOTAL_RETRY("zeppelin.websocket.total.retry", "5"),
    ZEPPELIN_WEBSOCKET_RETRY_TIME_SLEEP("zeppelin.websocket.retry.time.sleep", "3"),
    ZEPPELIN_WEBSOCKET_ENABLE_ASYNC_MESSAGE("zeppelin.websocket.enable.async.message", false),
    ...
    ZEPPELIN_INTERPRETER_LIMIT("zeppelin.interpreter.limit","bin/interpreter-limit.sh"),
    ZEPPELIN_INTERPRETER_LIMIT_ENABLED("zeppelin.interpreter.limit.enabled",false),
    ZEPPELIN_INTERPRETER_SKIP_ABORT("zeppelin.interpreter.skip.abort", true);
