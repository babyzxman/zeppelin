    ...
    new Thread(new Runnable() {
      ...
      public void run() {
        ...
        messagingService = new NettyMessagingService(
                "clusterName", // Cluster name or identifier
                address,
                new MessagingConfig()  // Default or custom configuration
        );
...
//                new MembershipConfig());
        ...
        //new version
        HeartbeatMembershipProtocolConfig protocolConfig = new HeartbeatMembershipProtocolConfig();
        ManagedClusterMembershipService clusterService = new DefaultClusterMembershipService(
                member,
                Version.from("1.0.0"),
                new DefaultNodeDiscoveryService(bootstrapService, member, new BootstrapDiscoveryProvider(clusterNodes)),
                bootstrapService,
                new HeartbeatMembershipProtocol(protocolConfig)
        );

        File atomixDateDir = com.google.common.io.Files.createTempDir();
        atomixDateDir.deleteOnExit();

        //removed withSerializer(storageSerializer)
