...
public class PySparkInterpreter extends PythonInterpreter {
  ...
  public InterpreterResult interpret(String st, InterpreterContext context)
    ...
    if (context.getLocalProperties().get("runBy") != null) {
      ...
      if (user != null && !user.equals("UNKNOWN")) {
        ...
        sparkInterpreter.getInnerInterpreter().getSparkContext().setLocalProperty("username", user);
        ...
        sparkInterpreter.getInnerInterpreter().getSparkContext().setLocalProperty("notebookId", context.getNoteId());
      }
    }
    String heraUrl = zConf.getString(ZeppelinConfiguration.ConfVars.HERA_ADDR)+":"+zConf.getString(ZeppelinConfiguration.ConfVars.HERA_PORT);
    sparkInterpreter.getInnerInterpreter().getSparkContext().setLocalProperty("heraUrl", heraUrl);
    sparkInterpreter.getInnerInterpreter().getSparkContext().setLocalProperty("refKey", zConf.getString(ZeppelinConfiguration.ConfVars.ZEPPELIN_APP_MODULE_REF_KEY));
    sparkInterpreter.getInnerInterpreter().getSparkContext().setLocalProperty("tenantId", zConf.getString(ZeppelinConfiguration.ConfVars.ZEPPELIN_APP_TENANT_ID));
  ...
  protected void preCallPython(InterpreterContext context) {
    ...
    String setPoolStmt = "if 'sc' in locals():\n\tsc.setLocalProperty('spark.scheduler.pool', " + pool + ")\n\t" +
            ...
            "sc.setLocalProperty('refKey', '" + zConf.getString(ZeppelinConfiguration.ConfVars.ZEPPELIN_APP_MODULE_REF_KEY) + "')\n\t" +
            ...
            "sc.setLocalProperty('username','" + context.getLocalProperties().get("runBy") + "')";
