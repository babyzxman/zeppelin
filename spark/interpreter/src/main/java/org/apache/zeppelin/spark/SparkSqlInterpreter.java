...
public class SparkSqlInterpreter extends AbstractInterpreter {
  ...
  public InterpreterResult internalInterpret(String st, InterpreterContext context)
    ...
    String timeZone = sqlContext.getConf("spark.sql.session.timeZone",null);
    if (!ObjectUtil.isNullOrEmpty(timeZone)) {
      LOGGER.info("Changed timezone to " + timeZone);
      TimeZone.setDefault(TimeZone.getTimeZone(timeZone));
    }
    else {
      if (!TimeZone.getDefault().getID().equals(this.timeZone)) {
          LOGGER.info("Changed timezone to " + this.timeZone);
