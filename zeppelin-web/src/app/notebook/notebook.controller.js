...
function NotebookCtrl($scope, $route, $routeParams, $location, $rootScope,
                      ...
                      heliumService, $window) {
  'ngInject';

  ngToast.dismiss();
  $scope.runValidate = false;
  $scope.statusToken = null;
  $scope.note = null;
  ...
  const initNotebook = function() {
    if($window.localStorage.getItem('sessionId') === undefined || $window.localStorage.getItem('sessionId') === null) {
      $window.localStorage.setItem('sessionId', $scope.generateUUID());
    }
    ...
    websocketMsgSrv.validateBDEToken($routeParams.noteId);
    $scope.$on('validateBDEToken', function(scope, event) {
      $scope.runValidate = true;
      if(event && event.status !== 'undefined') {
        if(event.status === 0) {
          $scope.username = event.username;
          $scope.permission = event.permission;
          $rootScope.username = event.username;
          $rootScope.permission = event.permission;
          $window.localStorage.setItem('user', event.username);
          $scope.moduleName = event.moduleNotebookName || '';
          if (!$scope.firstTime) {
            $rootScope.getNote = true;
            $scope.getNote = true;
  ...
  $scope.checkpointNote = function(commitMessage) {
    BootstrapDialog.confirm({
      ...
      title: '',
      message: `
        <img src="assets/images/dialog/commit-note.svg"/>
        <h3>Commit Note to\nCurrent Repository?</h3>
        <div class="desc-dialog">
          The Previous Note will be saved as\na Revision
          <span style="font-weight:bold;">'${commitMessage}'</span>
        </div>
      `,
      callback: function(result) {
        if (result) {
  ...
  $scope.setNoteRevision = function() {
    BootstrapDialog.confirm({
      ...
      title: '',
      message: `
        <img src="assets/images/dialog/set-notebook-head.svg"/>
        <h3>Set Notebook head to current revision?</h3>`,
      callback: function(result) {
        if (result) {
  ...
  $scope.runAllParagraphs = function(noteId) {
    BootstrapDialog.confirm({
      ...
      title: '',
      message: `
        <img src="assets/images/dialog/run-all-paragraph.svg"/>
        <h3>Run All Paragraphs?</h3>
        <div class="desc-dialog">Run all Paragraphs in this note.</div>`,
      callback: function(result) {
        if (result) {
  ...
  $scope.$on('addParagraph', function(event, paragraph, index) {
    // console.log("addParagraph", paragraph);
  ...
  $scope.restartInterpreter = function(interpreter) {
    let _dialog = new BootstrapDialog({
      title: '',
      ...
      buttons: [{
        ...
        action: function(dialog) {
          ...
          dialog.$modalFooter.find('button').addClass('disabled');
          dialog.$modalFooter.find('button:contains("OK")')
  ...
  $scope.checkRevision = function(revisionName) {
    $scope.revisionExists = false;
    for(let i=0; i<$scope.noteRevisions.length; i++) {
      if(revisionName === $scope.noteRevisions[i].message) {
        $scope.revisionExists = true;
      }
    }
  };
  ...
  $scope.$on('runAllAbove', function(event, paragraph, isNeedConfirm) {
    ...
    BootstrapDialog.confirm({
      ...
      title: '',
      message: `
      <img src="assets/images/dialog/run-all-above.svg"/>
      <h3>Run All Above?</h3>
      <div class="desc-dialog">Run all above Paragraph.</div>`,
      callback: function(result) {
        if (result) {
          websocketMsgSrv.runAllParagraphs($scope.note.id, paragraphs);
        }
  ...
  $scope.$on('runAllBelowAndCurrent', function(event, paragraph, isNeedConfirm) {
    ...
    BootstrapDialog.confirm({
      ...
      title: '',
      message: `
        <img src="assets/images/dialog/run-all-below.svg"/>
        <h3>Run Current and All Below?</h3>
        <div class="desc-dialog">Run current Paragraph and all below.</div>`,
      callback: function(result) {
        if (result) {
          websocketMsgSrv.runAllParagraphs($scope.note.id, paragraphs);
        }
