...
function ParagraphCtrl($scope, $rootScope, $route, $window, $routeParams, $location,
  ...
  $scope.$on('noteRunningStatus', function(event, status) {
    ...
    $scope.editor.setReadOnly(status || $scope.permission === 'viewer' || $scope.permission === 'public');
  });
  ...
  $scope.$watch($scope.getEditor, function(newValue, oldValue) {
    ...
    if ($scope.revisionView === true || $scope.isNoteRunning === true) {
      ...
      $scope.editor.setReadOnly(false || $scope.permission === 'viewer' || $scope.permission === 'public');
    }
  });
  ...
  $scope.removeParagraph = function(paragraph) {
    ...
    if ($scope.note.paragraphs.length === 1) {
      ...
      BootstrapDialog.confirm({
        ...
        message: `
          <img src="assets/images/dialog/remove-paragraph.svg"/>
          <h3>Remove This Paragraph?</h3>
          <div class="desc-dialog">
            This paragraph will be removed from your note.
          </div>`,
  ...
  $scope.aceLoaded = function(_editor) {
    ...
    if (_editor.container.id !== '{{paragraph.id}}_editor') {
      ...
      $scope.editor.setReadOnly($scope.isRunning($scope.paragraph) || $scope.isNoteRunning
        || $scope.permission === 'viewer' || $scope.permission === 'public');
  ...
  $scope.getExecutionTime = function(pdata) {
    ...
    let desc = `Took ${durationFormat}. Last updated at ${endFormat}.`;
  ...
  $scope.$on('keyEvent', function(event, keyEvent) {
    if ($scope.paragraphFocused) {
      ...
      if (editorHide && (keyCode === 38 || (keyCode === 80 && keyEvent.ctrlKey && !keyEvent.altKey))) { // up
        ...
        if($scope.permission !== 'viewer' && $scope.permission !== 'public') {
      ...
      } else if (keyEvent.ctrlKey && keyEvent.altKey && keyCode === 70) { // Ctrl + Alt + f
  ...
  $scope.$on('replaceCurrent', function(event, from, to) {
    ...
    // Commit paragraph for update replace text
    $scope.paragraph.text = $scope.editor.getValue();
