...
import org.apache.zeppelin.bde.services.hera.LicenseServices;
...
public class ManagedInterpreterGroup extends InterpreterGroup {
  ...
  private LicenseServices licenseService = new LicenseServices();

  private ZeppelinConfiguration zeppelinConf = ZeppelinConfiguration.create();
  ...
  public RemoteInterpreterProcess getOrCreateInterpreterProcess(String userName,
    ...
    synchronized (interpreterProcessCreationLock) {
      if (remoteInterpreterProcess == null) {
        try {
          if(zeppelinConf.getBoolean(ZeppelinConfiguration.ConfVars.ZEPPELIN_INTERPRETER_LIMIT_ENABLED)) {
            String scriptPath = zeppelinConf.getString(ZeppelinConfiguration.ConfVars.ZEPPELIN_INTERPRETER_LIMIT);
            ...
            LOGGER.info("Exit Status: " + exitCode);

            if (exitCode != 0) {
              throw new RuntimeException("Maximum number of interpreter instances reached. Please clean up existing interpreter processes before creating a new one.");
            }
          }
        }
        catch (Exception e) {
  ...
  public synchronized void close(String sessionId) {
    ...
    if (sessions.isEmpty() && interpreterSetting != null) {
      ...
      try {
        licenseService.sendSparkJobHealthStatus(null,null, new Date(System.currentTimeMillis()), null, true);
      } catch (Exception e) {
  ...
  public int getExitedSession() {
    return sessions.size();
  }

  public Map<String, List<Interpreter>> getSessions() {
    return sessions;
  }
