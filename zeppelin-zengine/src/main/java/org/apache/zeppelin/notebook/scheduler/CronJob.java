...
public class CronJob implements org.quartz.Job {
    ...
    try {
      notebook.processNote(noteId, note -> {
        ...
        if (note.getConfig().get("activatedBy") != null) {
          runBy = (String) note.getConfig().get("activatedBy");
        }
        ...
        AuthenticationInfo authenticationInfo =
                new AuthenticationInfo(
                        ...
                        null);
        try {
          note.runAll(authenticationInfo, true, true, runBy,true, new HashMap<>(), new HashMap<>());
  ...
  private boolean isIntpListHasSpark(List<InterpreterSetting> intpList){
    for(InterpreterSetting intp : intpList){
      if(intp.getName().contains("spark"))
        return true;
    }
    return false;
  }
