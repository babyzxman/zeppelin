...
public class GitNotebookRepo extends VFSNotebookRepo implements NotebookRepoWithVersionControl {
  ...
  public void remove(String noteId, String notePath, AuthenticationInfo subject)
    ...
    try {
      ...
      if (!pattern.matcher(notePath).find()) {
        git.rm().addFilepattern(noteFileName).call();
        git.commit().setMessage("Remove note: " + noteId + ", notePath: " + notePath).call();
        if (conf.getNotebookStorageClass().equals("org.apache.zeppelin.notebook.repo.GitHubNotebookRepo")) {
          String currentBranch = git.getRepository().getBranch();
          String remoteName = conf.getZeppelinNotebookGitRemoteOrigin();
          PushCommand pushCommand = git.push()
                  .setRemote(remoteName)
                  .add(currentBranch);
          pushCommand.setCredentialsProvider(
                  new UsernamePasswordCredentialsProvider(
                          conf.getZeppelinNotebookGitUsername(),
                          conf.getZeppelinNotebookGitAccessToken()
                  )
          );
          pushCommand.call();
        }
  ...
  public Revision checkpoint(String noteId,
    ...
    try {
      ...
      if (modified) {
        ...
        revision = new Revision(commit.getName(), commit.getShortMessage(), commit.getCommitTime(), false);
      } else {
  ...
  public List<Revision> revisionHistory(String noteId,
    ...
    try {
      ...
      for (RevCommit log: logs) {
        history.add(new Revision(log.getName(), log.getShortMessage(), log.getCommitTime(), false));
