...
public class Notebook {
  ...
  public String createNote(String notePath,
    ...
    return createNote(notePath, defaultInterpreterGroup, subject, noteId, true);
  }
  ...
  public String createNote(String notePath,
    ...
    Note note =
            new Note(notePath, defaultInterpreterGroup, replFactory, interpreterSettingManager,
                    paragraphJobListener, credentials, noteEventListeners, noteId);
  ...
  /**
   * Export existing ipynb note.
   *
   * @param noteId - the note ID to clone
   * @return Note JSON
   * @throws IOException, IllegalArgumentException
   */
  public String exportIpynbNote(String noteId) throws IOException {
    try {
      return processNote(noteId,
              note -> {
                if (note == null) {
                ...
                return new JupyterUtil().getNbformat(note.toJson());
              });
    } catch (IOException e) {
      throw new IOException("Note " + noteId + " not found");
    }
  ...
  public String importNote(String sourceJson, String notePath, AuthenticationInfo subject)
    ...
    if (oldNoteId!= null && getNotesInfo().stream().anyMatch(noteInfo -> noteInfo.getId().equals(oldNote.getId()))) {
        LOGGER.info("NoteId: " + oldNoteId + " already exits");
        oldNoteId = null;
    }
    String newNoteId = createNote(notePath,  oldNoteId, subject);
   ...
   *
   * @param sourceJson        - the note JSON to import
   * @param noteLanguage  - the language of note for use to bind default interpreter
   * @param notePath          - the path of the new note
   * @return noteId
   * @throws IOException
   */
  public String importNote(String sourceJson,String noteLanguage, String notePath, AuthenticationInfo subject)
          throws IOException {
    Note oldNote = Note.fromJson(null, sourceJson);
    if (notePath == null) {
      notePath = oldNote.getName();
    }
    String defaultIntpGroup = interpreterSettingManager.getByName(noteLanguage).getName();
    LOGGER.info("Default interpreter to bind to note : {}",defaultIntpGroup);

    String newNoteId = createNote(notePath, defaultIntpGroup, subject);
    processNote(newNoteId,
            newNote -> {
              List<Paragraph> paragraphs = oldNote.getParagraphs();
              for (Paragraph p : paragraphs) {
  ...
  public String cloneNote(String sourceNoteId, String revisionId, String newNotePath, AuthenticationInfo subject)
    ...
    return processNote(sourceNoteId,
      sourceNote -> {
        ...
        String newNoteId = createNote(newNotePath,subject, null, false);
        processNote(newNoteId,
          newNote -> {
  ...
  public String cloneNoteWithParameter(String sourceNoteId, String revisionId, String newNotePath,
                                       Map<String, Map<String, String>> parameters,
                                       Map<String, String> newInterpreterGroupName,
                                       String language, AuthenticationInfo subject)
          throws IOException {
      LOGGER.info("Start clone with param");
    return processNote(sourceNoteId,
            sourceNote -> {
              if (sourceNote == null) {
                throw new IOException("Source note: " + sourceNoteId + " not found");
              ...
              processNote(newNoteId,
                      newNote -> {
                        List<Paragraph> paragraphs = note.getParagraphs();
                        for (Paragraph p : paragraphs) {
                          newNote.addCloneNewParagraph(p, subject, parameters, newInterpreterGroupName, language);
                        }
                        newNote.setConfig(new HashMap<>(note.getConfig()));
                        newNote.setInfo(new HashMap<>(note.getInfo()));
                        newNote.setDefaultInterpreterGroup(note.getDefaultInterpreterGroup());
  ...
  public boolean isNotePathAvailable(String notePath){
    return noteManager.isNotePathAvailable(notePath);
  }
  ...
  public void reloadAllNotes(AuthenticationInfo subject) throws IOException {
    ...
    public void clearNoteRepo() throws IOException {
        noteManager.clearCache();
    }

    public void clearNoteRepoByNoteIds(List<String> noteIds) throws IOException {
        noteManager.clearCacheByNoteId(noteIds);
    }
