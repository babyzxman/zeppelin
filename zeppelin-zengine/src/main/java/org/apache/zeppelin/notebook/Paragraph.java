...
public class Paragraph extends JobWithProgressPoller<InterpreterResult> implements Cloneable,
  ...
  public void setScriptText(String newText)  {
    if (newText != null) {
      // clean localProperties, otherwise previous localProperties will be used for the next run
      ParagraphTextParser.ParseResult result = ParagraphTextParser.parse(newText);
  ...
  public boolean execute(String interpreterGroupId, boolean blocking) {
    try {
      ...
      // added noteId for isolate interpreter spark app name
      if (interpreterSetting.getOption().isIsolated()
              && !ObjectUtil.isNullOrEmpty(interpreter.getProperty("spark.app.name"))
              && !interpreter.getProperty("spark.app.name").contains("cron-")
              && !interpreter.getProperty("spark.app.name").contains(getNote().getId())){
        interpreter.setProperty("spark.app.name",interpreter.getProperty("spark.app.name") + "-" + getNote().getId());
      }
  ...
  private InterpreterContext getInterpreterContext() {
    ...
    localProperties.putIfAbsent("runBy", runBy);
